...
const AdminElderlyDetailPage = () => {
  ...
  const firstRegistrant = registeredPackages.length > 0 ? registeredPackages[0].registrant : null;
  ...
  const doctorCancelled = registeredPackages.filter(pkg => pkg.status === 'canceled' || pkg.status === 'cancelled').length;
  const supporterCompleted = supporterSchedules.filter(sch => sch.status === 'completed').length;
  const supporterCancelled = supporterSchedules.filter(sch => sch.status === 'canceled' || sch.status === 'cancelled').length;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 -mt-16 mb-6 relative z-10">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3">
                      ...
                      {age !== null && <span className="text-sm text-gray-600 font-medium">{age} tuổi</span>}
                      {firstRegistrant && (
                  ...
                  <div className="flex gap-3 w-full sm:w-auto">
                    <div className="flex-1 sm:flex-none bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200 text-center">
                      ...
                      <div className="text-2xl font-bold text-green-600 mt-1">{completedCount.toLocaleString()}</div>
                    </div>
                    <div className="flex-1 sm:flex-none bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-4 border border-red-200 text-center">
                      ...
                      <div className="text-2xl font-bold text-red-600 mt-1">{cancelledCount.toLocaleString()}</div>
                    </div>
                  </div>
                </div>

                {data.description && <p className="text-gray-700 leading-relaxed mb-6">{data.description}</p>}
                  ...
                  <div className="flex items-start gap-4 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-200">
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-gray-900 mt-1">
                        {emergencyContact?.name ? `${emergencyContact.name}` : (emergencyContact?.phone ?? "N/A")}
                      </div>
                      {emergencyContact?.phone && (
                        <div className="text-sm text-gray-600 mt-0.5">{emergencyContact.phone}</div>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              <div className="mb-4">
                <div className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Trạng thái tài khoản</div>
                <div className="mt-4 flex items-center gap-3">
                  <span
                    className={`h-4 w-4 rounded-full ${data.isActive ? "bg-green-500 shadow-lg shadow-green-500/50" : "bg-gray-400"}`}
                  ></span>
                ...
                {loadingPackages ? (
                  ...
                  <div className="space-y-4">
                    {usedPackages.map((pkg, idx) => (
                      <div key={pkg._id || idx} className="group relative rounded-xl border border-gray-200 bg-white hover:border-blue-300 hover:shadow-lg transition-all duration-300 overflow-hidden">
                          ...
                          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <h4 className="font-semibold text-gray-900 group-hover:text-blue-700 transition-colors">
                                  {pkg.packageRef?.title || "Gói khám không xác định"}
                                </h4>
                              </div>
                              <div className="text-sm text-gray-600 ml-12">
                                Đăng ký bởi: <span className="font-medium text-gray-900">{pkg.registrant?.fullName || "-"}</span>
                                {pkg.durationDays && (
                            ...
                            <div className="flex flex-wrap gap-3 text-sm">
                              <div className="flex items-center gap-2">
                                <span className="text-xs text-gray-500 hidden sm:inline">BS:</span>
                                {pkg.doctor?.fullName ? (
                                  <span className="px-3 py-1.5 rounded-lg bg-emerald-100 text-emerald-700 font-bold text-xs border border-emerald-200">
                                    {pkg.doctor.fullName}
                                  </span>
                                ) : (
                                  <span className="px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 text-xs">Chưa gán</span>
                                )}
                              </div>
                              <div className="text-gray-600">
                                Ngày: {pkg.registeredAt ? formatDate(pkg.registeredAt) : "N/A"}
                              </div>
                              <div className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">
                                {pkg.price ? pkg.price.toLocaleString("vi-VN") + " đ" : "N/A"}
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="text-center py-16 text-gray-500">
                    ...
                    <div className="font-medium">Chưa có gói khám nào đã sử dụng.</div>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="p-6">
                {registeredPackages.filter(p => p.doctor).length === 0 ? (
                  <div className="text-center py-16 text-gray-500">
                    ...
                    <div className="font-medium">Chưa có lịch hẹn với bác sĩ nào.</div>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {registeredPackages
                      ...
                      .map((pkg, idx) => (
                        <div key={pkg._id || idx} className="border border-emerald-100 rounded-xl p-5 bg-emerald-50/30 hover:bg-emerald-50 transition-colors">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-bold text-lg text-emerald-800">{pkg.packageRef?.title || "Gói khám"}</h4>
                              <div className="mt-2 text-sm text-gray-700">
                                <span className="font-medium">Bác sĩ phụ trách:</span> {pkg.doctor?.fullName || "Chưa chỉ định"}
                              </div>
                              <div className="text-sm text-gray-600">
                                Đăng ký ngày: {pkg.registeredAt ? formatDate(pkg.registeredAt) : "N/A"}
                              </div>
                            </div>
                            <span className="px-4 py-2 rounded-full bg-emerald-100 text-emerald-700 font-bold text-sm">
                              {pkg.status === "active" ? "Đang hoạt động" : pkg.status || "Chưa rõ"}
                            </span>
                          </div>
                        </div>
                      ))}
              ...
              <div className="p-6">
                {loadingSchedules ? (
                  <div className="text-center py-12">
                    ...
                    <div className="text-gray-600 mt-4 font-medium">Đang tải lịch hẹn...</div>
                  </div>
                  ...
                  <div className="text-center py-16 text-gray-500">
                    ...
                    <div className="font-medium">Chưa có lịch hẹn với người hỗ trợ.</div>
                  </div>
                )}
              </div>
            )}
