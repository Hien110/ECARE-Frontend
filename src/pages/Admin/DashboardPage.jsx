...
const DashboardPage = () => {
  ...
  const [supporterScheduledCount, setSupporterScheduledCount] = useState(0);

  // Filter data by selected month
  const filterByMonth = useCallback((items, dateField) => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        // Fetch all booking schedules (doctor + supporter)
        const supporterSchedulesRes = await adminService.getAllSupporterSchedules();
        if (mounted && supporterSchedulesRes && supporterSchedulesRes.success) {
          setSupporterBookings(supporterSchedulesRes.data || []);
        }

        const doctorSchedulesRes = await adminService.getAllDoctorSchedules();
        if (mounted && doctorSchedulesRes && doctorSchedulesRes.success) {
          setDoctorBookings(doctorSchedulesRes.data || []);
        }
        console.log(doctorSchedulesRes);
  ...
  // Update dashboard counters when schedules/month changes
  useEffect(() => {
    ...
    const supporterScheduled = filteredSupporterBookings.filter(b => ['pending', 'confirmed', 'in_progress'].includes(b?.status)).length;
    ...
    setSupporterScheduledCount(supporterScheduled);
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Chào mừng quay trở lại, Quản trị viên. Đây là trang thống kê khám bệnh.
          </p>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100">
            ...
            <p className="text-sm text-slate-600 mt-1">
              Thống kê lịch hẹn tư vấn và hỗ trợ chăm sóc sức khỏe theo tháng
            </p>
          </div>
          ...
          <div className="p-6">
            {(() => {
              const bars = [
                ...
                { id: "supporterCompleted", label: "Đã nhận chăm sóc", value: supporterCompletedCount, cls: "bg-emerald-600" },
                { id: "supporterScheduled", label: "Đã hẹn chăm sóc", value: supporterScheduledCount, cls: "bg-orange-500" },
              ];
              const maxValue = Math.max(...bars.map(b => b.value), 1);

              return (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-5 items-end h-72">
                    {bars.map((b) => (
                  ...
                  <div className="mt-4 text-center text-sm text-slate-500">
                    Số liệu {selectedMonth ? `tháng ${selectedMonth}` : "tất cả tháng"} -{" "}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-3xl font-extrabold">
                    {(
                      counts.totalResidents +
                      counts.familyMembers +
                      counts.activeSupporters +
                      counts.doctors
                    ).toLocaleString()}
                  </div>
                ...
                <div className="px-3 py-1 rounded-lg bg-white/15 text-sm font-semibold">
                  Tổng
                </div>
              </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-3xl font-extrabold text-slate-900 mt-1">
                {counts.familyMembers?.toLocaleString() ?? 0}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-3xl font-extrabold text-slate-900 mt-1">
                {counts.totalResidents?.toLocaleString() ?? 0}
              </div>
            </div>
          </div>
        </div>
